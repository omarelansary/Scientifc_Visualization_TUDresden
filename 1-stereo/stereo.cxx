...
enum ImageType { IT_COLOR, IT_DEPTH };

enum StereoRenderMode {
  SRM_INPUT, // shows the output of the per eye rendering passes (input to the
             // finalization pass)
...
/// anaglyph modes as explained on
...
enum NoiseType { NT_WHITE, NT_COLORED, NT_SPECKLE, NT_COLORED_SPECKLE };

class stereo : public cgv::base::node,
...
private:
  ...
  /// pointer to current view with support for scene extent based clipping plane
  /// (z_near/z_far) computation
...
protected:
  ...
  /// whether to automatically set parallax_zero_depth from distance to focus
  /// point
  ...
  /// index of to be rendered image (0 ... left, 1 ... right) for debugging
  /// modes (INPUT, REMAP, PARALLAX)
  int image_idx;
  /// image type of to be rendered image for debugging modes (INPUT, REMAP,
  /// PARALLAX)
  ImageType image_type;
  /// depth range and scale used for depth image visualization
  float min_depth, max_depth, depth_scale;
  /// fragments on far clipping plane are detected with z_far_epsion and
  /// discarded
  bool discard_fragments_on_far_clipping_plane;
  /// epsilon on depth used to check whether point is not on far clipping place
  /// in remapping
  float z_far_epsilon;
...
public:
  stereo()
      : node("stereo"), col_texs{texture("[R,G,B]"), texture("[R,G,B]")},
        dep_texs{texture("[D]"), texture("[D]")}, rnd_tex("[R,G,B]") {
    read_mesh = false;
    ...
    srs.material.set_brdf_type((cgv::media::illum::BrdfType)(
    ...
    for (unsigned i = 0; i < 100000; ++i) {
      ...
      box_colors.push_back(cgv::rgb(0.2f * D(R) + 0.7f * ctr[0] + 0.15f,
                                    0.2f * D(R) + 0.7f * ctr[1] + 0.15f,
                                    0.2f * D(R) + 0.7f * ctr[2] + 0.15f));
    }
    show_spheres = false;
    show_boxes = false;
  ...
  std::string get_type_name() const { return "stereo"; }

  /// makes data member of this class available as named properties (which can
  ...
  bool self_reflect(cgv::reflect::reflection_handler &rh) {
    return rh.reflect_member("file_name", file_name) &&
           ...
           rh.reflect_member("indirect_resolution_divisor",
                             indirect_resolution_divisor) &&
           ...
           rh.reflect_member("sync_parallax_zero_depth",
                             sync_parallax_zero_depth) &&
           ...
           rh.reflect_member("discard_fragments_on_far_clipping_plane",
                             discard_fragments_on_far_clipping_plane) &&
  ...
  void on_set(void *member_ptr) {
    if (member_ptr == &rnd_dot_divisor || member_ptr == &noise_type ||
        member_ptr == &speckle_percentage) {
  ...
  bool init(context &ctx) {
    ...
    finalize_prog.specify_standard_vertex_attribute_names(ctx, false, false,
                                                          true);
  ...
  void init_frame(context &ctx) {
    ...
    if (fbos[0].is_created() &&
        (resolution_changed || fbos[0].get_width() != w ||
         fbos[0].get_height() != h)) {
      for (int i = 0; i < 2; ++i) {
        fbos[i].destruct(ctx);
    ...
    if (!rnd_tex.is_created()) {
      ...
      case NT_WHITE: {
        ...
        for (unsigned i = 0; i < N; ++i)
          data_ptr[3 * i] = data_ptr[3 * i + 1] = data_ptr[3 * i + 2] =
              (cgv::type::uint8_type)D(G);
        break;
      }
      case NT_COLORED: {
        std::uniform_int_distribution<int> D(0, 255);
  ...
  void render_boxes(context &ctx) {
    ...
    attribute_array_binding::set_global_attribute_array(
        ctx, min_pnt_idx, &boxes[0].ref_min_pnt(), boxes.size(),
        sizeof(cgv::box3));
    attribute_array_binding::set_global_attribute_array(
        ctx, max_pnt_idx, &boxes[0].ref_max_pnt(), boxes.size(),
        sizeof(cgv::box3));
    attribute_array_binding::set_global_attribute_array(ctx, color_idx,
                                                        box_colors);
  ...
  void render_screen_filling_quad(context &ctx, shader_program &prog) {
    ...
    P.push_back(cgv::vec2(-1, -1));
    T.push_back(cgv::vec2(0.0f, 0.0f));
    P.push_back(cgv::vec2(1, -1));
    T.push_back(cgv::vec2(1.0f, 0.0f));
    P.push_back(cgv::vec2(-1, 1));
    T.push_back(cgv::vec2(0.0f, 1.0f));
    P.push_back(cgv::vec2(1, 1));
    T.push_back(cgv::vec2(1.0f, 1.0f));
  ...
  /// renders two spheres at the top of the viewport showing the current eye
  /// seperation
  void render_spheres(context &ctx) {
    ...
    cgv::dvec3 x_dir =
        normalize(cross(view_ptr->get_view_dir(), view_ptr->get_view_up_dir()));
    cgv::dvec3 y_dir = cross(x_dir, view_ptr->get_view_dir());
    double half_eye_distance = 0.5 * screen_width * eye_separation;
    double radius = 0.4;
    cgv::dvec3 center =
        view_ptr->get_focus() +
        (0.5 * screen_height - radius * srs.radius_scale) * y_dir;
  ...
  /// renders each view in seperate framebuffer and combines them in a third
  /// rendering pass
  void indirect_two_pass_stereo(context &ctx) {
    ...
    for (int i = 0; i < 2; ++i) {
      // current view (-1 = left & 1 = right) which can be used as sign in the
      // matrix computations
      int eye = 2 * i - 1;
      ...
      glClearColor(pow(clear_color[0], g), pow(clear_color[1], g),
                   pow(clear_color[2], g), 1);
      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

      /************************************************************************************
       tasks 1.1.a:  tranformation matrices
            Compute the correct model view and projection matrices for the
       current eye and set it with ctx.set_projection_matrix() and
       ctx.set_modelview_matrix(). You can get the matrices for the current
       camera position with ctx.get_projection_matrix() and
            ctx.get_modelview_matrix().
            You can use fmat<float, 4, 4> for matrix computations.
            You may have a look at the variables: eye, eye_separation,
       screen_width, screen_height, parallax_zero_depth, z_near, z_far.
       tasks 1.1.b:  cyclopic lighting
            Instead of using the lighting from both eye positions, set the
       correct projection and modelview matriices for cyclopic lighting. Use the
       variable 'cyclopic_lighting' to make it switch between both lightings */

      float z_0 = parallax_zero_depth;
      float l = -0.5f * screen_width * z_near / z_0;
      float r = 0.5f * screen_width * z_near / z_0;
      float b = -0.5f * screen_height * z_near / z_0;
      float t = 0.5f * screen_height * z_near / z_0;
      float delta = eye * 0.5f * eye_separation * screen_width * (z_near / z_0);
      cgv::mat4 MV = ctx.get_modelview_matrix();
      cgv::mat4 P = ctx.get_projection_matrix();
      // P = frustum4<float>(l + delta, r + delta, b, t, z_near, z_far);
      cgv::mat4 T = translate4<float>(-eye * 0.5f * eye_separation, 0.0f, 0.0f);
      cgv::mat4 S;
      S.identity();
      S(0, 2) = eye * 0.5f * eye_separation / z_0;
      ...
      // store per eye modelview projection matrix to hand over to finalization
      // pass
    ...
    // recover original viewport which is used to render the screen filling
    // rectangle
    ...
    finalize_prog.set_uniform(ctx, "discard_fragments_on_far_clipping_plane",
                              discard_fragments_on_far_clipping_plane);
  ...
  void draw(context &ctx) {
    ...
    /************************************************************************************
     tasks 1.4.c:  one pass rendering
          Exchange the inderect_two_pass_stereo() method with a one pass
     rendering method */
  ...
  void destruct(context &ctx) { ref_sphere_renderer(ctx, -1); }
  ...
  void stream_help(std::ostream &os) {
    os << "stereo: image <0|1> idx, <t>ype; <I|R|M|P|A|S> mode; <e>nables"
       << std::endl;
  }

  void stream_stats(std::ostream &os) {
    static const char *mode_names[] = {"input",    "random_dots",
                                       "remapped", "parallax",
                                       "anaglyph", "autostereogram"};
    static const char *anaglyph_names[] = {"true", "gray", "color",
                                           "half_color", "optimized"};
    os << "stereo: " << (image_type == IT_COLOR ? "color" : "depth") << "["
       << image_idx << "], mode = " << mode_names[mode]
       << ", anaglyph = " << anaglyph_names[anaglyph_mode] << std::endl;
  }
  ...
  void create_gui() {
    ...
    if (begin_tree_node("scene", file_name, true)) {
      align("\a");
      add_member_control(this, "translate_z", translate_z, "value_slider",
                         "min=0.0;max=1000;ticks=true;log=true");
      ...
      add_gui(
          ...
          "title='Open Mesh';filter='Mesh Files (obj):*.obj|All Files:*.*'");
    ...
    if (begin_tree_node("rendering", enabled, true)) {
      ...
      add_member_control(this, "indirect_resolution_divisor",
                         indirect_resolution_divisor, "value_slider",
                         "min=1;max=16;ticks=true");
      align("\b");
      end_tree_node(enabled);
    }
    if (begin_tree_node("stereo parameters", eye_separation, true)) {
      align("\a");
      add_member_control(this, "eye_separation", eye_separation, "value_slider",
                         "min=0;max=0.5;step=0.00001;ticks=true;log=true");
      add_member_control(this, "parallax_zero_depth", parallax_zero_depth,
                         "value_slider",
                         "min=0.001;max=1000;ticks=true;log=true");
      add_member_control(this, "sync_parallax_zero_depth",
                         sync_parallax_zero_depth, "toggle");
      add_member_control(this, "cyclopic_lighting", cyclopic_lighting,
                         "toggle");
      add_member_control(this, "z_near", z_near, "value_slider",
                         "min=0.001;max=1000;ticks=true;log=true");
      add_member_control(this, "z_far", z_far, "value_slider",
                         "min=0.001;max=1000;ticks=true;log=true");
    ...
    if (begin_tree_node("finalization pass", mode, true)) {
      align("\a");
      add_member_control(this, "mode", mode, "dropdown",
                         "enums='input,random_dots,remapped,parallax,anaglyph,"
                         "autostereogram'");
      add_member_control(this, "image_idx", (cgv::type::DummyEnum &)image_idx,
                         "dropdown", "enums='0,1'");
      add_member_control(this, "image_type", image_type, "dropdown",
                         "enums='color,depth'");
      add_member_control(this, "min_depth", min_depth, "value_slider",
                         "min=0;max=1;ticks=true");
      add_member_control(this, "max_depth", max_depth, "value_slider",
                         "min=0;max=1;ticks=true");
      add_member_control(this, "depth_scale", depth_scale, "value_slider",
                         "min=1;max=10000;ticks=true;log=true");
      add_member_control(this, "discard_fragments_on_far_clipping_plane",
                         discard_fragments_on_far_clipping_plane, "toggle");
      add_member_control(this, "z_far_epsilon", z_far_epsilon, "value_slider",
                         "min=0.0;step=0.00001;max=0.001;ticks=true;log=true");
      add_member_control(
          this, "remap_epsilon", remap_epsilon, "value_slider",
          "min=0.00001;step=0.0000001;max=0.1;ticks=true;log=true");
      add_member_control(this, "parallax_scale", parallax_scale, "value_slider",
                         "min=1;max=1000;step=0.00001;ticks=true;log=true");
      add_member_control(this, "anaglyph_mode", anaglyph_mode, "dropdown",
                         "enums='true,gray,color,half_color,optimized'");
      /*<your-code-here>*/
      align("\b");
      end_tree_node(mode);
    }
    if (begin_tree_node("noise", noise_type, false)) {
      align("\a");
      add_member_control(this, "noise_type", noise_type, "dropdown",
                         "enums='white,colored,speckle,colored_speckle'");
      add_member_control(this, "speckle_percentage", speckle_percentage,
                         "value_slider",
                         "min=0.0001;max=1;step=0.00001;log=true;ticks=true");
      add_member_control(this, "rnd_dot_divisor", rnd_dot_divisor,
                         "value_slider", "min=1;max=16;ticks=true");
      align("\b");
